...
module.exports = class PQMaxHeap {
  constructor() {
  ...
  popMax() {
    const poppedMax = this.queue.shift();
    ...
    while (left < this.queue.length) {
      let largestChildIdx = right < this.queue.length && this.queue[right].priority > this.queue[left].priority
        ? right
        : left;
      if (this.queue[largestChildIdx].priority > this.queue[currentNodeIdx].priority) {
        this._swap(currentNodeIdx, largestChildIdx);
        currentNodeIdx = largestChildIdx;
